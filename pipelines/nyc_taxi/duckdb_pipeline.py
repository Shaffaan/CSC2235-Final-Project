...
if __name__ == "__main__":
    ...
    config = {
        ...
        "num_threads": psutil.cpu_count(logical=True), "memory_limit": f"{int(psutil.virtual_memory().total / 1024**3 * 0.8)}GB",
        "data_size_pct": "20%", "system_size_pct": "100%"
    }
    
    run_optimized_pipeline(config, pd.DataFrame()).to_csv(os.path.join(RESULTS_DIR, 'duckdb_results.csv'), index=False)