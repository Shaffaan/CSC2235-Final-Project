...
def main():
    """
    Main orchestrator script to run all benchmarks, reconcile, and plot.
    """
    # --- 1. Setup ---
    ...
    try:
        ...
        spark = (
            ...
            .appName("FeatureEngineeringBenchmark")
            .master("local[*]")
        ...
        # Ensure Spark stops even on error
        try:
            spark.stop()
        except:
            pass
    ...
    # --- 3. Reconcile Results ---
    if len(results) < 5:
        print("\n--- 3. Skipping Reconciliation (not all benchmarks completed) ---")
    else:
