...
def calculate_and_save_stats(df, config):
    ...
    nulls_before = df.filter(F.col("fare_amount").isNull()).count()
    pct_before = (nulls_before / total_rows) * 100 if total_rows > 0 else 0.0

    # Nulls AFTER imputation
    nulls_after = df.filter(F.col("fare_amount_imputed").isNull()).count()

    # Rows imputed
    imputed_rows = df.filter(
        F.col("fare_amount").isNull() |
        (F.col("fare_amount") <= 0) |
        (F.col("fare_amount") != F.col("fare_amount_imputed"))
    ).count()
...
def inject_nulls(df, config):
    ...
    df_nulls = df.withColumn(
        target_col,
        F.when(F.rand() < null_pct, F.lit(None)).otherwise(F.col(target_col))
    )

    df_nulls = df_nulls.cache()
    df_nulls.count()
