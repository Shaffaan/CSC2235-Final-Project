...
def execute_pipeline(config):
    print("  Executing C4 Pipeline (Pandas)...")
    
    all_chunks = []
    
    for f in config['data_files']:
        print(f"    Reading {os.path.basename(f)}...")
        # CRITICAL: convert_dates=False to keep timestamp as string for slicing
        df = pd.read_json(f, lines=True, compression='gzip', convert_dates=False)
        
        # 1. Filter
        df = df[df['text'].astype(str).str.len() > 100].copy()
        
        # 2. Transform (Extract and Lowercase)
        df['domain'] = df['url'].astype(str).str.extract(r'https?://([^/]+)', expand=False).str.lower()
        df['year'] = df['timestamp'].astype(str).str.slice(0, 4)
        
        # Drop Nulls
        df = df.dropna(subset=['domain'])
        
        chunk_agg = df.groupby(['domain', 'year']).size().reset_index(name='count')
    ...
    if not all_chunks:
        return pd.DataFrame(columns=['domain', 'year', 'count'])

    full_df = pd.concat(all_chunks, ignore_index=True)
    
    # 3. Aggregate & Sort
    final_agg = full_df.groupby(['domain', 'year'])['count'].sum().reset_index()
    final_agg = final_agg.sort_values(['count', 'domain'], ascending=[False, True]).head(50)
...
if __name__ == "__main__":
    RESULTS_DIR = os.environ.get('SCRIPT_RESULTS_DIR') or "results/local_test/allenai_c4/pandas"
    ...
    run_pipeline(config, pd.DataFrame()).to_csv(os.path.join(RESULTS_DIR, 'pandas_results.csv'), index=False)