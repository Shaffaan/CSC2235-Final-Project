...
class OpenImagesConfig:
    ...
    def resolve_results_dir(self) -> str:
        ...
        if env_dir:
            return env_dir
        return os.path.join(PROJECT_ROOT, "results", "local_test", "cv", "openimages_pandas")


def ensure_openimages_assets(config: OpenImagesConfig) -> Tuple[str, str]:
    """Downloads the detection annotations and class metadata if missing."""
...
def load_detections(csv_path: str, limit: Optional[int]) -> pd.DataFrame:
...
def load_class_descriptions(csv_path: str) -> pd.DataFrame:
...
def compute_box_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["box_width"] = np.clip(df["XMax"] - df["XMin"], 0.0, None)
    df["box_height"] = np.clip(df["YMax"] - df["YMin"], 0.0, None)
    df["box_area_ratio"] = (df["box_width"] * df["box_height"]).clip(lower=0.0)

    if {"ImageWidth", "ImageHeight"}.issubset(df.columns):
        df["ImageWidth"] = df["ImageWidth"].astype(float)
        df["ImageHeight"] = df["ImageHeight"].astype(float)
        df["image_area_px"] = (df["ImageWidth"] * df["ImageHeight"]).replace({0.0: np.nan})
        df["box_area_px"] = (
            df["box_width"] * df["ImageWidth"] * df["box_height"] * df["ImageHeight"]
        )
        df["normalized_area"] = (df["box_area_px"] / df["image_area_px"]).fillna(0.0)
    else:
        df["image_area_px"] = np.nan
        df["box_area_px"] = np.nan
        df["normalized_area"] = df["box_area_ratio"]

    df["normalized_area"] = df["normalized_area"].clip(0.0, 1.0)
    df["size_bucket"] = pd.cut(
        df["normalized_area"],
        bins=AREA_BUCKET_EDGES,
...
def attach_class_names(detections: pd.DataFrame, class_df: pd.DataFrame) -> pd.DataFrame:
    merged = detections.merge(class_df, on="LabelName", how="left")
    merged = merged.rename(columns={"DisplayName": "class_name", "LabelName": "class_id"})
    return merged


def summarize_class_areas(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    area_stats = (
        df.groupby(["class_id", "class_name"], dropna=False)
        .agg(
            detections=("ImageID", "count"),
            mean_norm_area=("normalized_area", "mean"),
            median_norm_area=("normalized_area", "median"),
            std_norm_area=("normalized_area", "std"),
        )
        .reset_index()
    ...
    bucket_counts = (
        df.groupby(["class_id", "class_name", "size_bucket"], dropna=False)
        .size()
        .reset_index(name="detections")
        .sort_values(["class_id", "size_bucket"])
    )

    return area_stats, bucket_counts


def summarize_bucket_distribution(df: pd.DataFrame) -> pd.DataFrame:
    return (
        df.groupby("size_bucket")
        .agg(
            detections=("ImageID", "count"),
            avg_normalized_area=("normalized_area", "mean"),
        )
        .reset_index()
...
def run_openimages_pipeline(config: OpenImagesConfig) -> Dict[str, Any]:
    ...
    start = time.perf_counter()
    outputs = persist_results(config.resolve_results_dir(), area_stats, bucket_counts, bucket_summary)
    _record_timing(timings, "persist_results", start)

    timings_path = persist_timings(timings, config.resolve_results_dir())
...
def parse_args() -> Tuple[OpenImagesConfig, Optional[str]]:
    parser = argparse.ArgumentParser(description="Open Images bounding box statistics with Pandas")
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--annotation-file",
        default="oidv6-train-annotations-bbox.csv",     # FIXED
        help="Detection annotations CSV filename to download/use.",
    )

    parser.add_argument(
        "--class-file",
        default="oidv6-class-descriptions.csv",         # FIXED
    ...
    parser.add_argument(
        "--results-dir",
        default=None,
        help="Directory to store CSV summaries (defaults to results/local_test/cv/openimages_pandas).",
    )
    parser.add_argument(
        "--presets",
        default=None,
...
def persist_preset_summary(records: List[Dict[str, Any]], summary_dir: str) -> Optional[str]:
    ...
    summary_path = os.path.join(summary_dir, "openimages_pandas_preset_summary.csv")
