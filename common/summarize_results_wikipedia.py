...
def _normalize_config_label(config_name: str) -> str:
    """Returns a unified wikipedia_{size} label regardless of execution framework."""
    if not isinstance(config_name, str):
        return config_name

    lowered = config_name.lower()
    for prefix in ("wikipedia_spark_", "wikipedia_"):
        if lowered.startswith(prefix):
            suffix = lowered[len(prefix) :]
            return f"wikipedia_{suffix}"
    return lowered
...
def _aggregate_join_metrics(join_df: pd.DataFrame, plots_dir: str):
    ...
    # Explicit size ordering controls x-axis layout (largest first).
    size_order = {
        "mega": 5,
        "full": 6,
        "xlarge": 4,
    ...
    summary["config_label"] = summary["config_name"].apply(_normalize_config_label)
    config_order = sorted(summary["config_label"].dropna().unique(), key=sort_key)
    page_rows_map = (
        summary.dropna(subset=["config_label"])
        .groupby("config_label")["page_rows"]
        .max()
        .to_dict()
    )
    axis_label_map = {
        label: (
            f"{label} ({int(page_rows_map[label]):,} pages)"
            if label in page_rows_map and pd.notna(page_rows_map[label])
            else label
        )
        for label in config_order
    }
    axis_order = list(axis_label_map.values())

    for join_type in sorted(join_df["join_type"].dropna().unique()):
        subset = summary[summary["join_type"] == join_type].copy()
        if subset.empty:
            continue
        subset["config_label"] = pd.Categorical(subset["config_label"], categories=config_order, ordered=True)
        subset = subset.sort_values("config_label")
        subset["config_axis"] = subset["config_label"].map(axis_label_map)
